...
const fetchDataForLocation = async (name: string): Promise<Location> => {
    ...
    if (dbLocation) {
        ...
        const liveAQI = await fetchLiveAQI(dbLocation.latitude, dbLocation.longitude);
        ...
        return {
            ...
            pollutants: liveAQI?.pollutants ?? (latestReading ? {
            ...
            lastUpdated: liveAQI?.timestamp
                ...
                : (latestReading ? new Date(latestReading.recorded_at).toLocaleTimeString() : 'Recently'),
        };
    }
    ...
    try {
        ...
        if (suggestions.length > 0) {
            ...
            return {
                ...
                lastUpdated: liveAQI?.timestamp ? new Date(liveAQI.timestamp).toLocaleTimeString() : 'Just now',
            };
        }
    } catch (e) {
    ...
    const fallbackLat = name.toLowerCase() === 'mumbai' ? 19.0760 : (name.toLowerCase() === 'pune' ? 18.5204 : 28.6139);
    const fallbackLon = name.toLowerCase() === 'mumbai' ? 72.8777 : (name.toLowerCase() === 'pune' ? 73.8567 : 77.2090);
    ...
    return {
        ...
        lastUpdated: fallbackAQI?.timestamp ? new Date(fallbackAQI.timestamp).toLocaleTimeString() : 'Just now',
    };
};
                ...
                <AnimatePresence>
                    {isLoading && (
                        ...
                        >
                            ...
                            <p className="text-zinc-400 font-medium">Fetching real-time data...</p>
                        </motion.div>
                    )}
                </AnimatePresence>
                            ...
                            <div className="flex items-center gap-5">
                                ...
                                <div className="text-left">
                                    ...
                                    <div className="flex items-center gap-2 text-zinc-400 mt-1">
                                        ...
                                        <span className="text-xs font-mono bg-white/5 px-2 py-0.5 rounded">{liveSelectedLocation.lat.toFixed(4)}, {liveSelectedLocation.lng.toFixed(4)}</span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="lg:col-span-2 space-y-8">
                                    ...
                                    >
                                        ...
                                        <div className="flex flex-col justify-center space-y-5 relative z-10">
                                            <div className="space-y-2 text-left">
                                                <p className="text-sm font-bold uppercase tracking-widest text-zinc-500">Current Quality</p>
                                    ...
                                    >
                                        {[
                                            ...
                                            { name: 'NO2', value: liveSelectedLocation.pollutants?.no2 || 23.1, unit: 'ppb', description: 'Nitrogen dioxide, primarily from vehicle emissions and power plants.' },
                                            { name: 'SO2', value: liveSelectedLocation.pollutants?.so2 || 8.4, unit: 'ppb', description: 'Sulfur dioxide, produced from burning fossil fuels.' },
                                            { name: 'CO', value: liveSelectedLocation.pollutants?.co || 1.2, unit: 'ppm', description: 'Carbon monoxide, colorless odorless gas from combustion.' },
                                            { name: 'O3', value: liveSelectedLocation.pollutants?.o3 || 56.8, unit: 'ppb', description: 'Ground-level ozone, formed from vehicle emissions and sunlight.' },
                                        ].map((pollutant) => (
                                                        ...
                                                        {fireRisk.hotspots.slice(0, 4).map((fire, idx) => (
                                                            <div key={idx} className="flex items-center justify-between text-xs pb-3 border-b border-white/5 last:border-0 last:pb-0">
                                                                ...
                                                                <div className="text-right">
                                                                    <div className="font-black text-orange-500">{fire.frp} MW</div>
                                    ...
                                    >
                                        <Card className="p-6 bg-gradient-to-br from-teal-600 to-cyan-600 border-none overflow-hidden relative group">
                                            ...
                                            <div className="relative z-10 space-y-4">
                                                <h3 className="font-black text-xl leading-tight text-white">Join the Network</h3>
                                                <p className="text-teal-100 text-sm">Contribute hyper-local data from your neighborhood by installing a VAYU IoT node.</p>
                                                <Button className="bg-white text-teal-700 hover:bg-teal-50 w-full font-bold shadow-lg">Inquire About Hardware</Button>
                                            </div>
