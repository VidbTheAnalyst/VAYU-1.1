            ...
            <Card className="overflow-hidden bg-white/5 backdrop-blur-xl border border-white/10">
                <table className="w-full text-left">
                    <thead className="bg-white/5 border-b border-white/10">
                        <tr>
                            <th className="px-6 py-4 text-sm font-semibold text-zinc-400">Pollutant</th>
                            <th className="px-6 py-4 text-sm font-semibold text-white">{loc1.name}</th>
                            <th className="px-6 py-4 text-sm font-semibold text-white">{loc2.name}</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-white/10">
                        {pollutants.map((pollutant) => {
                            ...
                            return (
                                <tr key={pollutant.name} className="hover:bg-white/5 transition-colors">
                                    <td className="px-6 py-4">
                                        <span className="text-sm font-medium text-white">{pollutant.name}</span>
                                        <span className="ml-1 text-[10px] text-zinc-500 uppercase tracking-tighter">({pollutant.unit})</span>
                                    </td>
                                    <td className={`px-6 py-4 text-sm ${isLoc1Worse ? 'font-bold underline decoration-red-400 decoration-2 underline-offset-4' : 'text-zinc-400'}`}>
                                        {pollutant.loc1Value}
                                    </td>
                                    <td className={`px-6 py-4 text-sm ${!isLoc1Worse ? 'font-bold underline decoration-red-400 decoration-2 underline-offset-4' : 'text-zinc-400'}`}>
                                        {pollutant.loc2Value}
